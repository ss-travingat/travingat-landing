...
import { users, explorerCards, waitlist } from "@/db/schema";
...
export async function POST(req: Request) {
  try {
    ...
    if (profilePublicUrl) userUpdateData.avatar_url = profilePublicUrl;
    if (coverPublicUrl) userUpdateData.cover_photo_url = coverPublicUrl;
    ...
    const existingCards = await db.select({ userId: explorerCards.userId })
      .from(explorerCards)
      .where(eq(explorerCards.userId, userId))
      .limit(1);
    const isFirstTime = existingCards.length === 0;

    // Upsert into the new explorer_cards table
    await db.insert(explorerCards)
      .values({
        userId,
      ...
      .onConflictDoUpdate({
        target: explorerCards.userId,
        set: {
          name: `${firstName} ${lastName}`,
          country,
          visitedCountries,
          profileImageUrl: profilePublicUrl || undefined,
          coverImageUrl: coverPublicUrl || undefined,
          profileOriginalExt: profileOriginalExt || undefined,
          coverOriginalExt: coverOriginalExt || undefined,
          cardStyle: cardStyle || '--',
          cardCreated: true,
        },
      });
